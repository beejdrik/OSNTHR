...
void dijkstraAlg::executeAlgorithm(int source, int destination) {
    ...
    while (!priorityQueue.isEmpty()) {
       int currentVertex = priorityQueue.pop()->second;
        ...
        if (currentVertex == destination) {     //Found the best route to the final destination, we can stop looking now
            return;
        }
        ...
        //next-neighbor (cost) as newDistance. If it's shorter than the currently best known distance to that vertex,
